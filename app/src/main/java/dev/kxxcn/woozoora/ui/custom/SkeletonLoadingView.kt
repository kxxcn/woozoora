...
class SkeletonLoadingView @JvmOverloads constructor(
    ...
    val BASE_COLOR_DEFAULT = context.color(R.color.shimmerBase)
    val DEEP_COLOR_DEFAULT = context.color(R.color.shimmerHighlight)
    val RADIUS_DEFAULT = 10.dpToPx.toFloat()
    val PROGRESS_LENGTH_DEFAULT = 120.dpToPx.toFloat()
        ...
        with(context.obtainStyledAttributes(attrs, R.styleable.SkeletonLoadingView)) {
            if (hasValue(R.styleable.SkeletonLoadingView_radius))
                radius = getDimensionPixelOffset(
                    R.styleable.SkeletonLoadingView_radius,
                    RADIUS_DEFAULT.toInt()
                ).toFloat()
            if (hasValue(R.styleable.SkeletonLoadingView_duration))
                durationOfPass = getInt(
                    R.styleable.SkeletonLoadingView_duration,
                    DURATION_DEFAULT.toInt()
                ).toLong()
            if (hasValue(R.styleable.SkeletonLoadingView_interval))
                interval = getInt(
                    R.styleable.SkeletonLoadingView_interval,
                    INTERVAL_DEFAULT.toInt()
                ).toLong()
            ...
            if (hasValue(R.styleable.SkeletonLoadingView_progressLength))
                progressLength = getDimensionPixelOffset(
                    R.styleable.SkeletonLoadingView_progressLength,
                    PROGRESS_LENGTH_DEFAULT.toInt()
                ).toFloat()
            ...
            canvas.drawRoundRect(
                ...
                deepPaintEnd.apply {
                    shader.setLocalMatrix(
                        m.apply {
                            setTranslate(
                                screenWidth * frame - x + progressLength / 2 - screenWidth, 0f
                            )
                        }
                    )
                }
